...
- **In scope**:
  ...
  - 複数レシートの一括アップロードおよび解析キュー管理（解析順序制御、レシート切り替え、スキップ、削除）
  ...
  - キュー完了時の画面遷移制御（1件以上登録時は登録完了画面の表示、全スキップなど登録0件時は初期トップ画面への静かな復帰）
...
5. When ユーザーが現在アクティブなレシートのスキップを指示したとき, the Receipt Workflow UI shall 当該レシートの登録処理を行わずに次の未処理レシートへ遷移する
...
### Requirement 4: Zaim への支出登録と完了フロー
**Objective:** As a ユーザー, I want 編集した明細をワンクリックで Zaim に登録し、二重登録を未然に防ぎつつ、全スキップ時も迷わず初期状態に戻りたい, so that 正確な家計簿記録をスムーズかつ安心して残せる
...
5. When キュー内のすべてのレシートの処理が終了した時点で1件以上の支出が正常に登録されていたとき, the Receipt Workflow UI shall 登録完了画面（完了ステート）を表示する
6. When キュー内のすべてのレシートがスキップされ登録された支出が0件のままキューの処理が終了したとき, the Receipt Workflow UI shall 登録完了画面を表示せず、静かに初期（トップ）画面へ遷移する
